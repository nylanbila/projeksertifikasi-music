...
$result_artikel = $koneksi->query("SELECT COUNT(*) as total FROM articles");
$total_artikel = $result_artikel->fetch_assoc()['total'];

$result_komentar = $koneksi->query("SELECT COUNT(*) as total FROM comments");
$total_komentar = $result_komentar->fetch_assoc()['total'];

$result_user = $koneksi->query("SELECT COUNT(*) as total FROM users");
  ...
  <div class="max-w-6xl mx-auto">
      ...
      <div class="bg-gradient-to-br from-indigo-500 to-violet-600 text-white rounded-xl shadow p-5">
        <div class="flex justify-between items-center">
          <div>
            ...
            <h2 class="text-3xl font-bold mt-1"><?= $total_artikel ?></h2>
          </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div class="bg-white rounded-xl p-6 shadow hover:shadow-md transition">
        ...
        <a href="comment_management.php" class="text-indigo-600 text-sm font-medium mt-3 inline-block hover:underline">
          Lihat Komentar →
        </a>
      </div>
