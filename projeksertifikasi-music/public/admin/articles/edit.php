...
// ambil data
$stmt = mysqli_prepare($koneksi, "SELECT id, title, content, featured_image FROM articles WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($errors)) {
    $stmt = mysqli_prepare($koneksi, "UPDATE articles SET title = ?, content = ?, featured_image = ?, updated_at = NOW() WHERE id = ?");
