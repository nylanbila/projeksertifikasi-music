...
$stmtChk = $koneksi->prepare("SELECT id FROM articles WHERE id = ? LIMIT 1");
...
// toggle like
$stmt = $koneksi->prepare("SELECT id FROM likes WHERE article_id = ? AND user_id = ? LIMIT 1");
...
if ($res->num_rows > 0) {
  ...
  $del = $koneksi->prepare("DELETE FROM likes WHERE id = ?");
  ...
  // like
  $stmt->close();
  $ins = $koneksi->prepare("INSERT INTO likes (article_id, user_id) VALUES (?, ?)");
